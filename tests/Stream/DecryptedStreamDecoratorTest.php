...
class DecryptedStreamDecoratorTest extends TestCase
{
    ...
    public function testReadWithStreamData(): void
    {
        ...
        $this->stream->method('read')
            ->willReturnCallback(function($length) {
                ...
                if ($callCount === 2) {
                    return str_repeat('E', self::BLOCK_SIZE) . str_repeat('X', self::MAC_SIZE);
                }
    ...
    {
        $encryptedBlock = str_repeat('x', self::BLOCK_SIZE);
        $mac = str_repeat('y', self::MAC_SIZE);
        $finalChunk = $encryptedBlock . $mac;
